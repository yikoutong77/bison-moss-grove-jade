...
export function GameOverScreen() {
  ...
  return (
    <div className="tavern-shell flex h-full flex-col px-4 pt-3 pb-4">
      <button
        type="button"
        className="action-btn primary mx-auto w-full max-w-lg shrink-0"
        onClick={mode === "online" ? leaveRoom : startSelect}
      >
        <RotateCcw className="size-4" />
        再来一局
      </button>
      <div className="mx-auto mt-3 min-h-0 w-full max-w-lg flex-1 overflow-y-auto">
        <div className="panel rounded-2xl p-5">
          <div className="flex flex-col items-center text-center">
            <Crown className={cn("size-9", place === 1 ? "text-gold" : "text-muted")} />
